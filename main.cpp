...
void OnMapRequest(const xcb_generic_event_t* NextEvent) {
    ...
    uint32_t AttributesMasks[] = {XCB_EVENT_MASK_ENTER_WINDOW | XCB_EVENT_MASK_FOCUS_CHANGE | 0x0000ff};
    ...
    xcb_change_window_attributes_checked(WM.Connection, Event->window, XCB_CW_EVENT_MASK | XCB_CW_BORDER_PIXEL, &AttributesMasks);
    xcb_configure_window(WM.Connection, Event->window, ConfigureMasks, Parameters);
    
    //xcb_change_window_attributes(WM.Connection, Event->window, XCB_CW_BORDER_PIXEL, 0xff0000);
